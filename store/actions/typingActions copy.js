...
// Fetch Typing Records with Pagination
export const fetchTypings = createAsyncThunk(
   "typing/fetchTypings",
   async ({ page = 1 }, thunkAPI) => {
      try {
         // 🚀 Pass the page number to your Laravel/Next API
         const res = await axios.get(`/api/typing/fetchTypings?page=${page}`);
         return res.data;
         // Expected Laravel Response: { data: [...], current_page: 1, last_page: 10 }
      } catch (error) {
         return thunkAPI.rejectWithValue(
            error.response?.data?.message || "Failed to fetch Typings"
         );
      }
   }
);
