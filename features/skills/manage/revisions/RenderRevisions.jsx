...
const RenderRevisions = ({
  ...
  const filteredRevisions = useMemo(() => {
    ...
    return revisions.filter((rev) => {
      ...
      // Check Skill Level
      const matchSkill =
        category?.skillId === activeRouteId ||
  ...
  if (filteredRevisions.length === 0) {
    return (
      <div className="p-4 text-gray-500 italic">
        No revisions scheduled for this category branch.
      </div>
    );
  }
          ...
          <div className="border border-gray-700 rounded-lg overflow-hidden bg-gray-900/20">
            <table className="min-w-full border-collapse">
              <thead className="bg-gray-800">
                <tr>
                  <th className="border border-gray-700 px-4 py-2 text-left text-xs font-bold text-gray-400 uppercase w-12">
                    S.No
                  </th>
                  <th className="border border-gray-700 px-4 py-2 text-left text-xs font-bold text-gray-400 uppercase">
                    Topic
                  </th>
                  <th className="border border-gray-700 px-4 py-2 text-center text-xs font-bold text-gray-400 uppercase">
                    Scheduled
                  </th>
                  <th className="border border-gray-700 px-4 py-2 text-center text-xs font-bold text-gray-400 uppercase">
                    Practiced
                  ...
                  <th className="border border-gray-700 px-4 py-2 text-center text-xs font-bold text-gray-400 uppercase w-24">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody>
                {items.map((rev, i) => (
                  ...
                  >
                    <td className="border border-gray-700 px-4 py-2 text-sm text-gray-500">
                      {i + 1}
                    </td>
                    ...
                    <td className="border border-gray-700 px-4 py-2">
                      <div className="flex justify-center gap-4">
                        <Edit2
                          ...
                          size={16}
                          className="cursor-pointer hover:text-blue-400 text-gray-500"
                        />
                        <Trash2
                          onClick={() => handleDelClick(rev)}
                          size={16}
